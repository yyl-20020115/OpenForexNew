//+------------------------------------------------------------------+
//|                                                         OFxP.mq5 |
//|                              Copyright Copyright 2010, NOC Corp. |
//|                                                 http://www.noc.cn|
//+------------------------------------------------------------------+
#property copyright "Copyright 2010, NOC Corp."
#property link      "http://www.noc.cn"
...
bool OrderClose( int ticket, double lots, double price, int slippage, color Color=0) 
{
  return false;
}
bool OrderModify( int ticket, double price, double stoploss, double takeprofit, datetime expiration, color arrow_color=0) 
{
  return false;
}
...
int  MODE_DIGITS = 0;
int  MODE_SPREAD=0;
int  MODE_STOPLEVEL=0;
int  MODE_LOTSIZE=0;
int  MODE_MINLOT=0;
int  MODE_TICKVALUE=0;
int  MODE_TICKSIZE=0;
int  MODE_SWAPLONG=0;
int  MODE_SWAPSHORT=0;
int  MODE_STARTING=0;
int  MODE_EXPIRATION=0;
int  MODE_TRADEALLOWED=0;
int  MODE_LOTSTEP=0;
int  MODE_MAXLOT=0;
int  MODE_SWAPTYPE=0;
int  MODE_PROFITCALCMODE=0;
int  MODE_MARGINCALCMODE=0;
int  MODE_MARGININIT=0;
int  MODE_MARGINMAINTENANCE=0;
int  MODE_MARGINHEDGED=0;
int  MODE_MARGINREQUIRED=0;
int  MODE_FREEZELEVEL=0;
int  MODE_HISTORY = 0;
...
string DoubleToStr(double d, int digits = 0)
{
...
extern bool e_smartValueUpdate = false;
...
string GetExpertId()
{
   ...
   {// Testing mode = different ID.
      return (Symbol() + "(Test." + IntegerToStr(Period()) + ")");
   }
   
   return (Symbol()  + "(" + IntegerToStr(Period()) + ")");
}
...
int start()
{
   ...
   if (g_statusOK == false)
   {
      if (IsConnected() == false)
      {
         Trace("Expert at " + Symbol() + "." + IntegerToStr(Period()) + "not connected.", false);
      }
      else
      {
         Trace("Expert at " + Symbol() + "." + IntegerToStr(Period()) + "stopped (may be not properly started and not functioning).", false);
      }
   }
...
bool ParseRequestNewOrder(string& symbol, double& volume, double& prefferedPrice, double& slippage, double& takeProfit, double& stopLoss, int& orderType, int& operationID, string& comment)
{
   ...
   orderType = (int)StrToInteger(results[6]);
...
bool ParseRequestValues(int& operationID, int& prefferedValueCount, string& symbol)
{
   ...
   Trace("Requesting values [" + IntegerToStr(prefferedValueCount) + "].", false);
...
void SendAllOrders(int operationId)
{
   ...
   for(int pos=0; operationResult > 0 && pos < totalHistoricalOrders; pos++)
   {
      //if(OrderSelect(pos,SELECT_BY_POS, MODE_HISTORY) == false) 
      //TODO: need to correct
      if(OrderSelect(pos) == false) 
      {// Failed to select order or order belongs to different symbol - do not touch it.
         Trace("Order select by position error.", true);
         continue;
      }
...
int ToSlippagePoints(string symbol, double value)
{
   ...
   int result = (int)(value / MarketInfo(symbol, MODE_POINT));
...
void HandleNewOrderRequests()
{
   ...
   {
      Trace("OpeningOrder volume[" + IntegerToStr(volume) + "] prefferedPrice[" + DoubleToStr(prefferedPrice) + "] slippagePoints[" + DoubleToStr(slippagePoints) + "] takeProfit[" + DoubleToStr(takeProfit) + "] stopLoss[" + DoubleToStr(stopLoss) + "] orderType[" + IntegerToStr(orderType) + "] operationId[" + IntegerToStr(operationId) + "] comment[" + comment + "]", false);
      ...
      int compensatedSlippagePoints = (int)ToSlippagePoints(symbol, compensatedSlippage);
      ...
      {// Price slipped too much already, so just place the original call instead of the optimized one (since the optimized is now invalid).
         ...
         compensatedSlippagePoints = (int)slippagePoints;
      }

      Trace("SendingOrder orderType[" + IntegerToStr(orderType) + "] volume[" + IntegerToStr(volume) + "] prefferedPrice[" + DoubleToStr(prefferedPrice) + "] currentPrice[" + DoubleToStr(currentPrice) + 
         "] requestSlip[" + DoubleToStr(slippagePoints) + "pts >> " + DoubleToStr(slippage) + "] currentSlip[" + DoubleToStr(currentSlippage) + "] appliedSlip ["+ DoubleToStr(compensatedSlippage)+ " >> " + IntegerToStr(compensatedSlippagePoints) + "pts"
         + "] takeProfit [" + DoubleToStr(takeProfit) + "] stopLoss[" + DoubleToStr(stopLoss) + "]  operationId[" + IntegerToStr(operationId) + "] comment[" + comment + "]", false);
      ...
      {// Order ok.
         ...
         Trace("Order opened Time[" + TimeToStr(orderOpeningTime) + "], Price[" + IntegerToStr(orderOpeningPrice) + "].", false);
      }
...
void HandleCloseOrderRequests()
{
   ...
   {
      Trace("Closing Order ticket[" + IntegerToStr(orderTicket) + "] closingVolume[" + IntegerToStr(closingVolume) + "] prefferedPrice["+ DoubleToStr(prefferedPrice) + "] slippagePts[" + DoubleToStr(slippagePoints) + "]", false);
...
*/
      ...
      Trace("ClosingOrder closeVolume[" + IntegerToStr(closingVolume) + "] prefferedPrice[" + DoubleToStr(prefferedPrice) + "] currentClosePrice[" + DoubleToStr(currentClosingPrice) + 
         "] requestSlip[" + DoubleToStr(slippagePoints) + "pts >> " + DoubleToStr(slippage) + "] currentSlip[" + DoubleToStr(currentSlippage) + "] appliedSlip ["+ DoubleToStr(compensatedSlippage)+ " >> " + IntegerToStr(compensatedSlippagePoints) + "pts"
         + "]  operationId[" + IntegerToStr(operationId) + "]", false);
      ...
      {// Order properly closed.
         ...
         Trace("Order close at " + DoubleToStr(orderClosedAtPrice), false);
...
*/
      ...
      {// Found it, this is our new order.
         Trace("Order found : " + IntegerToStr(OrderTicket()), false);
         return (OrderTicket());
      }
   }

   Trace("Order (by comments) not found [" + IntegerToStr(oldOrderTicket) + "].", true);
   return (-1);
}
...
void HandleModifyOrderRequests()
{
   ...
   {
      Trace("ParseRequestModifyOrder" + IntegerToStr(orderTicket) + "; " + IntegerToStr(operationId) + "; " + DoubleToStr(stopLoss) + "; " + DoubleToStr(takeProfit) + "; " + DoubleToStr(targetOpenPrice) + "; " + IntegerToStr(expiration), false);
...
void SendTradingValuesHistory(int operationId, int prefferedValueCount)
{
   ...
   double ratesArray[][6];
 //TODO: fix
   int arrayCopyResultCount = 0;//ArrayCopyRates(ratesArray);
   if (arrayCopyResultCount == -1)
   {// Error - copying went wrong.
      ReportError("Failed to copy needed data, communication with DLL failed. Error is #" + IntegerToStr(arrayCopyResultCount) + ", " + PrintErrorDescription(arrayCopyResultCount), -1);
      return;
   }
   ...
   {
      ReportError("ERROR - There is size difference at array resize. Size provided was : " + IntegerToStr(resizeResult) + " / " + IntegerToStr((6 * arrayCopyResultCount)), -1);
      return;
   }
